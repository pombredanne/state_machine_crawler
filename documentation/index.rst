...
transitions between states blindly but finds the shortest path between two states based on the *cost* of all possible
paths between them. The ability to find the path between multiples states comes from a representation of the states as a
`directed graph <http://en.wikipedia.org/wiki/Directed_graph>`_. The model allows to employ pure mathematical algorithm
to measure distance between the nodes.
